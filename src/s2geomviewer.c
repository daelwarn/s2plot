...
void CreateProjection(int eye) {
  ...
  case PERSPECTIVE:
    ...
    // set _s2mpi_clipview to 1 before call to clip the view
    // corresponding to state in drawView (geomviewer.c).
    // clip the view for world coordinate drawing
    // do not clip the view for screen coord drawing
    if (_s2mpi_clipview) {
